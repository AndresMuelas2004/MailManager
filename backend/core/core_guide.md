...
1. **Shared helpers first** — check `helpers.py` for functions that already solve the problem (`parse_expiry`, `unwrap_app_credentials`, `unwrap_user_tokens`, `wrap_account_tokens`, `http_error_detail`, `inline_cid_images`, `decode_mime_body`). `inline_cid_images(html, cid_map)` replaces `src="cid:<id>"` and `background="cid:<id>"` references with data URLs from a `{content_id: data_url}` dict; tolerant to single/double/no quotes and to optional angle brackets around the CID; unmapped CIDs are left untouched (soft fallback — a broken inline image is better than losing the whole email body). `decode_mime_body(data_b64url, charset_hint)` decodes a base64url-encoded MIME body into a Python `str` using a **UTF-8-first strategy with validated fallback**: strict UTF-8 first, fall back to the declared charset only when UTF-8 fails (catches senders mislabelling UTF-8 as Latin-1/Windows-1252 — the classic mojibake bug), last resort UTF-8 with `errors="replace"`. Returns `None` only when the base64 payload itself is invalid. Used by Gmail's MIME body extractor; Outlook doesn't need it because Graph decodes server-side. If the new logic is useful to more than one client, extract it to `helpers.py`.
...
- **`fetch_email_content(provider_message_id) → EmailContent`** — fetch the full body (HTML and/or plain text) for a single email, with inline `cid:` images already resolved to `data:` URLs in the returned HTML. Gmail requires MIME tree traversal and base64url decoding via the private helper `_extract_body_from_payload` because the Gmail API (`format="full"`) returns the raw MIME structure with nested `parts[]` and base64url-encoded bodies. The helper extracts the charset declared in each part's `Content-Type` header and delegates the actual base64 + string decoding to the shared helper `core.email.helpers.decode_mime_body(data_b64url, charset_hint)`. That helper implements a **UTF-8-first strategy with validated fallback**: it first tries strict UTF-8 (any real UTF-8 body decodes cleanly — Python raises `UnicodeDecodeError` on invalid continuation bytes, so a genuine Latin-1 body can never be misdecoded here); on `UnicodeDecodeError` it falls back to the declared charset (e.g. `iso-8859-1`, `windows-1252`, `big5`) with `errors="replace"`; on `LookupError` (unknown charset) or as last resort, it decodes UTF-8 with `errors="replace"`. This prevents the mojibake class of bugs where senders mislabel UTF-8 bodies as Latin-1 and the old blind-trust-the-header decoder produced `Ã©` instead of `é`. `decode_mime_body` returns `None` only if the base64 payload itself is invalid. Outlook's Graph API returns `body.content` pre-decoded as a JSON string, so Outlook doesn't need `decode_mime_body` — the charset negotiation is handled server-side. When `html_body` is non-empty, Gmail performs a second MIME walk via `_extract_cid_attachments(payload, provider_message_id)` that collects every `image/*` part with a `Content-ID` header, decodes inline `body.data` (base64url) directly, or makes an extra `users().messages().attachments().get()` call when the part carries only `body.attachmentId`; the resulting `{content_id: data_url}` map is handed to the shared `inline_cid_images` helper, which resolves `cid:` references in HTML attributes (`src=`/`background=`) **and** inside CSS `url(cid:…)` functions (e.g. `style="background-image:url(cid:…)"` produced by premailer when it inlines `<style>` rules). Per-image failures (HTTP errors, decode errors, unexpected exceptions) are soft-fallbacks — the CID is logged with `logger.warning` and left unrewritten in the HTML. Outlook's Graph API (`$select=body,hasAttachments`) returns the decoded HTML/text content directly — no body decoding needed. When `body.contentType == "html"` and `hasAttachments` is true, `_fetch_inline_image_cids(escaped_message_id)` issues `GET /me/messages/{id}/attachments?$select=contentType,contentBytes,contentId,isInline` and builds a `{contentId: data_url}` map from entries where `isInline` is true, `contentId` is populated and `contentType` starts with `image/`; `contentBytes` is already standard base64 (no re-decoding) so the data URL is assembled directly, then passed to `inline_cid_images`. An `EmailExternalAPIError` or unexpected exception during the attachments fetch is a soft fallback — the HTML is returned with unresolved `cid:` references instead of failing the request. Both providers raise `EmailNotAuthenticatedError` if not authenticated and `EmailExternalAPIError` on provider-level body-fetch failures.
...
- **`update_draft(provider_draft_id, to_recipients, cc_recipients, bcc_recipients, subject, body_html) → DraftMetadata`** — replace an existing draft's content at the provider (full-field replacement; every field is sent and overwrites the previous value). Gmail reuses `_build_draft_raw_message` (the same helper used by `create_draft`) to produce the base64url-encoded MIME payload and calls `users().drafts().update(userId="me", id=provider_draft_id, body={"message": {"raw": ...}})`. Gmail preserves `draft.id` on update — the client returns the same `provider_draft_id` it received. Note: the inner `message.id` within the draft resource may change after an update call (Gmail API behavior); the implementation discards the update response, so this rotation has no effect. Outlook reuses `_build_draft_graph_payload` and calls `PATCH /me/messages/{id}` **with the `Prefer: IdType="ImmutableId"` header repeated on every call** — the stored `provider_draft_id` is an Immutable ID, so Graph must be told again on each subsequent call to interpret the path parameter correctly. Both raise `EmailNotAuthenticatedError` if not authenticated and `EmailExternalAPIError` on provider failures. Timestamps in the returned `DraftMetadata` are best-effort (Gmail does not return them on update; Outlook does, but the service layer ignores them — the DB is the source of truth for `created_at`/`updated_at`).
...
- **`send_draft(provider_draft_id) → EmailMetadata`** — send an existing draft at the provider. Both implementations retry transient failures up to 3 total attempts (`_SEND_DRAFT_MAX_ATTEMPTS = 3`, `_SEND_DRAFT_RETRY_DELAY = 1.0`). Gmail calls `users().drafts().send(userId="me", body={"id": provider_draft_id})` which returns a `Message` resource with a **new** `message_id` (different from the draft ID) — the draft is automatically deleted by Gmail. Outlook calls `POST /me/messages/{provider_draft_id}/send` with `Prefer: IdType="ImmutableId"` — returns 202 (no body); the message ID stays the **same** thanks to ImmutableId. Both use best-effort post-send metadata enrichment via `fetch_messages_metadata` (same pattern as `send_email`), falling back to minimal `EmailMetadata(box="SENT")` if enrichment fails. Gmail retries only on `_RETRYABLE_STATUS_CODES` (429, 5xx); Outlook retries all `EmailExternalAPIError` up to the limit.
...
### Manager-level — `EmailManager.send_draft(account_label, provider_draft_id) → EmailMetadata`

Routes to the client matching `account_label` via `_get_client_or_raise` and calls `client.send_draft(provider_draft_id)`. Standard error handling: `CoreError` re-raised as-is, other exceptions wrapped in `EmailExternalAPIError`.
...
- [ ] Implement all abstract methods: `authenticate`, `authenticate_silent`, `fetch_email_metadata`, `send_email`, `send_draft`, `verify_message_existence`, `delete_messages`, `restore_from_trash`, `fetch_messages_metadata`, `move_to_trash`, `update_read_status`, `move_to_spam`, `restore_from_spam`, `fetch_email_content`, `create_draft`, `update_draft`, `delete_draft`, `fetch_drafts`, `get_account_label`.
