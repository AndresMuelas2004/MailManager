"""Move emails to trash for a specific account under a mailbox."""
...
from api.services.emails_service import move_to_trash


def main() -> None:
    parser = argparse.ArgumentParser(description="Move emails to trash.")
    ...
    parser.add_argument("--message-ids", required=True,
                        help="Comma-separated provider message IDs to move to trash.")
    ...
    payload = MoveToTrashRequest(items=items)

    print(f"Moving {len(items)} email(s) to trash for account '{args.account_id}' "
          f"in mailbox '{args.mailbox_id}'...")
    result = move_to_trash(args.mailbox_id, payload, owner_user_id)
