"""Move emails to spam for a specific account under a mailbox."""
...
from api.services.emails_service import move_to_spam


def main() -> None:
    parser = argparse.ArgumentParser(description="Move emails to spam.")
    ...
    parser.add_argument("--message-ids", required=True,
                        help="Comma-separated provider message IDs to move to spam.")
    ...
    print(f"Moving {len(items)} email(s) to spam for account '{args.account_id}' "
          f"in mailbox '{args.mailbox_id}'...")
    result = move_to_spam(args.mailbox_id, payload, owner_user_id)
    print(f"Done. Moved: {result.moved_count}")
