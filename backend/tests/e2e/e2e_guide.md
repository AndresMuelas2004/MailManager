...
### Section 5f: Draft deletion — pre-existing accounts (tests 43–44)

| Test | Endpoint | Dependencies |
|---|---|---|
| 43 | `DELETE .../accounts/{aid}/drafts/{draft_id}` (gmail) | independent — creates a draft, then deletes it via the endpoint, verifies gone from DB and provider via sync |
| 44 | `DELETE .../accounts/{aid}/drafts/{draft_id}` (outlook) | independent — creates a draft, then deletes it via the endpoint, verifies gone from DB and provider via sync |
...
8. **The draft is intentionally left at the provider** — same pattern as tests 32/33. The `DELETE /mailboxes/{mid}/accounts/{aid}/drafts/{draft_id}` endpoint is now available and tested separately in Section 5f (tests 43–44).
...
6. Cleanup local row via raw SQL in a `finally` block — the draft stays at the provider (same pattern as tests 32/33). Provider-side draft deletion is covered separately by tests 43–44 in Section 5f.
