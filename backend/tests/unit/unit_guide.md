...
`test_drafts_service.py` contains two classes:
...
- **`TestListDrafts`** covers `drafts_service.list_drafts` (DB-only listing). Tests include: single-account happy path (asserts `created_at DESC` ordering on fake rows), unified view happy path (3 rows from `list_by_mailbox`), account not found (`AccountNotFound`), mailbox access denied (`Forbidden`), DB error on account lookup (`DatabaseQueryError`), DB error on `list_by_account` and `list_by_mailbox` (`DatabaseQueryError`), unexpected `RuntimeError` on all three DB call sites (`DraftListError`), empty result returns `[]`, and `None`-field coalescing in `DraftOut`.
- Both classes share the module-level `_fake_account` and `_persisted_row` helpers. `TestCreateDraft` uses its own `_patch_common` (patches `draft_store.create`), while `TestListDrafts` uses `_patch_list_common` (patches `draft_store.list_by_account` and `list_by_mailbox` instead).
