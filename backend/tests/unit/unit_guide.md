...
- `update_draft_exc`, `update_draft_return: DraftMetadata | None`, `update_draft_calls` — mirror the create draft injection kwargs for the update operation. `update_draft_calls` records a 6-tuple `(provider_draft_id, to, cc, bcc, subject, body_html)` — note the extra leading `provider_draft_id` element compared to the 5-tuple used by `create_draft_calls`.
- `delete_draft_exc` — raise on `delete_draft` calls.
- `delete_draft_calls` — `list[str]` field that records every `delete_draft` invocation. Each entry is the `provider_draft_id` string passed to the call (simpler than the tuple format used by `create_draft_calls` and `update_draft_calls`).
- `send_draft_exc`, `send_draft_return: EmailMetadata | None`, `send_draft_calls` — mirror the create/update draft injection kwargs for the send-draft operation. `send_draft_calls` records each invocation as a bare `provider_draft_id: str` (not a tuple), because send-draft takes only the draft ID. `send_draft_return` returns an `EmailMetadata` (the sent message), not a `DraftMetadata`.
...
`test_drafts_service.py` contains five classes:
...
- **`TestUpdateDraft`** covers `drafts_service.update_draft` end-to-end at the unit level. Tests include (non-exhaustive): happy path (verifies returned `DraftOut` carries the updated recipients, subject, and body from the payload), pre-check `DraftNotFound` when `draft_store.get` returns `None`, account not found, mailbox access denied (`Forbidden`), DB errors on `account_store.get` and `draft_store.get` (both translated via `translate_database_error`), silent auth `EmailAuthError` → `AccountNotConnected`, provider `EmailExternalAPIError` → `ExternalAPIError`, provider generic `RuntimeError` → `DraftUpdateError`, DB persist `DatabaseError` on `draft_store.update` (translated), DB persist generic `RuntimeError` → `DraftUpdateError`, outer safety net (unexpected error outside all inner try blocks → `DraftUpdateError`), payload field forwarding via 6-tuple `update_draft_calls` (first element is `provider_draft_id`, then to/cc/bcc/subject/body_html — differs from `create_draft_calls` 5-tuple), and `_persist_refreshed_tokens` behavior (same three paths as `TestCreateDraft`). The helper `_patch_update_common` extends `_patch_common` by additionally patching `draft_store.get` (to return a fake existing row) and `draft_store.update` (to return a deterministic `_persisted_row()`).
- **`TestDeleteDraft`** covers `drafts_service.delete_draft` end-to-end at the unit level (15 tests). Tests include: two happy-path tests (one asserting the return value is `{"status": "deleted"}`, one verifying provider is called before DB via `delete_draft_calls == [_DELETE_DRAFT_ID]`), draft not found in local DB (`DraftNotFound`), account not found, mailbox access denied (`Forbidden`), provider `EmailExternalAPIError` translation, provider generic `RuntimeError` (manager wraps to `EmailExternalAPIError`), silent auth `EmailAuthError` → `AccountNotConnected`, DB delete `DatabaseError` translation, DB error on `draft_store.get` translation, DB error on `account_store.get` translation, `_persist_refreshed_tokens` happy path (refreshed tokens persisted via `upsert_tokens`), `_persist_refreshed_tokens` `DatabaseError` translation, `_persist_refreshed_tokens` generic exception → `DraftDeleteError`, outer safety net (`load_wrapped_app_credentials` raising → `DraftDeleteError`), and provider error preserves local draft row — provider-failure tests assert `deleted == []` (the capture list returned by `_patch_delete_common`) to verify `draft_store.delete` is not called.
- **`TestSendDraft`** covers `drafts_service.send_draft` end-to-end at the unit level. Tests include (non-exhaustive): happy path (verifies `DraftSendOut` with `provider_message_id`, `provider`, `status="sent"`), account not found, draft not found (pre-check), mailbox access denied (`Forbidden`), provider `EmailExternalAPIError` → `ExternalAPIError`, provider generic `RuntimeError` → `ExternalAPIError` (via EmailManager wrapping), silent auth `EmailAuthError` → `AccountNotConnected`, DB draft delete failure swallowed (still returns success), metadata persist failure swallowed (still returns success), account DB lookup `DatabaseError` translation, account DB lookup generic `RuntimeError` → `DraftSendError`, draft DB lookup `DatabaseError` translation, draft DB lookup generic `RuntimeError` → `DraftSendError`, `_persist_refreshed_tokens` happy path / DB error / unexpected (validates `DraftSendError` fallback), and outer safety net (`load_wrapped_app_credentials` raising → `DraftSendError`). The helper `_patch_send_common` extends `_patch_common` by additionally patching `draft_store.get` (for the pre-check), `draft_store.delete` (no-op), and `persist_email_metadata_batch` (no-op).
- All six classes share the module-level `_fake_account` and `_persisted_row` helpers. `TestCreateDraft` uses its own `_patch_common` (patches `draft_store.create`), `TestListDrafts` uses `_patch_list_common`, `TestSyncDrafts` uses `_patch_sync_common` (patches `draft_store.replace_all_for_account` + sets up `FakeEmailClient.fetch_drafts_return`), `TestUpdateDraft` uses `_patch_update_common` (patches `draft_store.get` + `draft_store.update`), `TestDeleteDraft` uses `_patch_delete_common` (patches `draft_store.get` and `draft_store.delete`; returns a `deleted: list[tuple[str, str]]` capture list so tests can assert `deleted == []` when the provider call fails), and `TestSendDraft` uses `_patch_send_common` (patches `draft_store.get` + `draft_store.delete` + `persist_email_metadata_batch`).
...
`test_draft_repository.py` covers `PgDraftStore` with six test classes:

- **`TestPgDraftStoreCreate`** — happy path, psycopg2 error → `QueryError`, generic exception → `QueryError`, and **`ConnectionPoolError` (a `DatabaseError` subclass) propagates unchanged** (validates the `except DatabaseError: raise` guard — without it, pool exhaustion would be silently re-wrapped as `QueryError`).
- **`TestPgDraftStoreGet`** — 6 tests: happy path (row found → dict), `None` when row is missing, `None` on `InvalidTextRepresentation` (malformed UUID gracefully returns `None` instead of raising), `QueryError` on generic `psycopg2.Error`, `QueryError` on unexpected generic exception, and `ConnectionPoolError` propagation unchanged.
- **`TestPgDraftStoreUpdate`** — 5 tests: happy path (row updated → dict with refreshed `updated_at`), `QueryError("Draft row to update not found.")` when `UPDATE ... RETURNING` yields no row (race condition after the pre-check), `QueryError` on generic `psycopg2.Error`, `QueryError` on unexpected generic exception, and `ConnectionPoolError` propagation unchanged.
...
- **`TestPgDraftStoreGet`** — happy path, row not found → `None`, `InvalidTextRepresentation` → `None`, psycopg2 error → `QueryError`, generic exception → `QueryError`, `ConnectionPoolError` propagates unchanged.
- **`TestPgDraftStoreDelete`** — happy path, psycopg2 error → `QueryError`, generic exception → `QueryError`, `ConnectionPoolError` propagates unchanged.
