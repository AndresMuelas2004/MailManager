...
- All four classes share the module-level `_fake_account` and `_persisted_row` helpers. `TestCreateDraft` uses its own `_patch_common` (patches `draft_store.create`), `TestListDrafts` uses `_patch_list_common`, `TestSyncDrafts` uses `_patch_sync_common` (patches `draft_store.replace_all_for_account` + sets up `FakeEmailClient.fetch_drafts_return`), and `TestUpdateDraft` uses `_patch_update_common` (patches `draft_store.get` + `draft_store.update`).
