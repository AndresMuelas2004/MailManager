...
- **HTML sanitization**: the pipeline lives in its own module `api/services/email_html_pipeline.py` as a sequence of pure functions composed by `prepare_email_html(html: str) -> str`. `services_helpers.sanitize_email_html` is a thin re-export of that entry point for backwards compatibility with existing callers and tests. The pipeline runs seven steps in order: (1) `_normalize_charset_meta` strips any declared `<meta charset>` / `<meta http-equiv="Content-Type">` and, **only when a `<head>` exists**, injects a canonical `<meta charset="utf-8">` right after the opening `<head ...>` tag so lxml (premailer) and html5lib (bleach) treat the Python `str` as UTF-8 — without this, Outlook's bogus `charset=us-ascii` / `charset=windows-1252` meta would cause the parsers to re-interpret UTF-8 bytes as Latin-1 (mojibake on accented characters). When the input has no `<head>`, no canonical meta is injected: a Python `str` is already Unicode-decoded and prepending a `<meta>` before any `<!DOCTYPE>` would push lxml into quirks mode. (2) `_unwrap_mso_conditionals` keeps the payload of downlevel-revealed conditionals (`<!--[if !mso]><!-- -->…<!--<![endif]-->` and the compact `<!--[if !mso]><!-->…<!--<![endif]-->` short form) and **discards** downlevel-hidden ones entirely (`<!--[if mso | IE]>…<![endif]-->`, `<!--[if gte mso 9]><xml>…</xml><![endif]-->`). Downlevel-hidden blocks are Outlook-only by design and the iframe viewer is a non-Outlook client, so they must stay invisible. Unwrapping them was the cause of visible duplication whenever templates shipped both a desktop MSO variant and a non-MSO variant of the same hero/logo (Medusa Festival #20, Santander Open Academy #73). When discarding hidden blocks leaves an effectively empty body (more than 200 bytes of MSO-only content discarded **and** fewer than 50 visible characters of text remaining), `_unwrap_mso_conditionals` logs `logger.warning("MSO-only email detected: discarded N bytes …")` so legacy MSO-only newsletters become observable instead of disappearing silently — no automatic fallback is applied (silent visual duplication is worse than an empty-but-loggable body). (3) `_sanitize_style_blocks` parses every `<style>` block with cssutils, filters declarations against `_ALLOWED_CSS_PROPERTIES`, rejects property values matching `_UNSAFE_CSS_VALUE_RE` (`expression(…)`, `javascript:`, `vbscript:`), and keeps only the at-rules in `_ALLOWED_CSS_AT_RULES` (`@media`, `@supports`, `@font-face`) — `@import`, `@namespace`, `@charset`, `@keyframes` and anything unknown are dropped. Each rule is evaluated inside its own `try/except` so a single unparseable sibling rule (`calc(100% - )`, vendor-specific syntax, etc.) drops only that rule instead of wiping the whole block; previously, wiping the block took down preheader-hiding rules like `.preheader{display:none}` and caused the preheader-visible bugs (#5, #22, #51, #73, #84, #143, #315). The at-rule decision uses a `{rule_type: at_rule_name}` map consulted against `_ALLOWED_CSS_AT_RULES` (single source of truth — change the constant, not the function). This step is also what makes responsive newsletter templates render correctly inside the iframe viewer: with the `<style>` block stripped, mobile-first templates (eDreams, Netcapital, HubSpot-style) used to show only the preheader because the `@media (min-width:…)` override where the desktop layout lived was lost. (4) `_inline_css_via_premailer` runs `premailer.transform(keep_style_tags=True, remove_classes=False, cssutils_logging_level="CRITICAL", disable_validation=True)` so inlinable rules are applied as `style=""` attributes for mail-client compatibility **and** the sanitised `<style>` tag survives with `@media` queries / pseudo-classes intact. (5a) `_flatten_document_wrappers` strips head-only constructs that must not leak into the body fragment — `<!DOCTYPE>`, `<title>…</title>` (otherwise bleach would keep the inner text and render the subject as visible body content — Pencil.dev #36), `<meta>`, `<link>`, `<base>`, stray `<xml>` islands (MSO leftovers), plus the `<html>`/`<head>`/`<body>` wrapper tags themselves. When the original `<body>` carried a `style`/`bgcolor`, those values are promoted into a synthetic wrapping `<div style="…">` so the iframe's own `body{background:#fff}` reset does not cover HubSpot/Netcapital's outer background (bugs #8, #9). The wrapper's `style` attribute is escaped with `html.escape(quote=True)` so a `<body>` style that mixes single AND double quotes (e.g. ``font-family:"Helvetica Neue",'Arial'``) cannot break the wrapper attribute — `CSSSanitizer` cleans the CSS later. Without this head-only cleanup lxml's `fragment_fromstring` in the next step mis-interpreted the fragment and silently dropped real body content (Artlist #56, Eurofirms #473). (5b) `_mirror_geometry_to_attributes` walks the tree with lxml and mirrors `width`/`height` values from inline styles back onto HTML attributes on `<img>`/`<td>`/`<th>`/`<table>`, giving signature tables and inline logos a fallback dimension even if the style is lost later. The defensive guard counts critical layout elements (`img`/`td`/`th`/`table`) before and after the lxml reparse via the regex `_CRITICAL_ELEMENT_RE`; if the post-reparse count drops below half the input count, the step is abandoned and the input HTML is returned unchanged, so the geometry mirror can never cost us body content. (Counting elements rather than bytes avoids false positives caused by lxml's cosmetic normalisations — entity escaping, attribute requoting — that change byte length without losing content.) (6) `_strip_script_blocks` removes `<script>` blocks along with their inner text (bleach's `strip=True` would keep the text, which the browser would execute). `<style>` is *not* stripped anymore — its content was sanitised in step 3. (7) `_clean_with_bleach` applies the final allowlist pass: `_ALLOWED_TAGS` now includes `"style"`, `_ALLOWED_PROTOCOLS` covers `http`, `https`, `mailto`, `cid`, `data` (cid so unresolved inline-image references survive as soft fallback, data so images already resolved to base64 by `inline_cid_images` pass through), and inline `style=""` attributes go through `CSSSanitizer(allowed_css_properties=_ALLOWED_CSS_PROPERTIES, allowed_svg_properties=frozenset())` (requires `tinycss2`). Each step is fail-soft: on any unexpected error the step logs `logger.warning` and returns its input unchanged, so bleach always receives well-formed content and no step can take down the endpoint. `premailer`, `cssutils`, `lxml`, and `bleach.css_sanitizer.CSSSanitizer` are lazy-imported inside their respective steps. Applied before persisting to DB.
