...
- **Database** (`api/database/`) — PostgreSQL persistence layer. The interfaces `MailboxStore` / `AccountStore` in `base.py` are stable contracts. `repository.py` implements them with SQL queries. `token_store.py` handles account tokens in the `tokens` table and app credentials from file-based env vars. All public symbols are re-exported from the package `__init__.py` so consumers import from `api.database`. See `backend/api/database/DATABASE.md` for a detailed description of each file.
...
- Persistence uses PostgreSQL via `psycopg2`. Connection pool is managed in `api/database/db.py` with lazy initialisation and `init_db()` called at application startup via FastAPI lifespan.
